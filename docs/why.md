...
`go-ruby-rails/rails` reimplements Ruby's `rails` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-rails/rails` directly, with no Ruby runtime;
...
Because `rails` is pure Go with cgo disabled, it also compiles to **WebAssembly**
